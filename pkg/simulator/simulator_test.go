...
// TestBindingPropagation verifies that the scheduler correctly sets Pod.Spec.NodeName.
// This test uses the fake client, so it may still have the Binding issue.
// For reliable results, use a real cluster (kind) for experiments.
...
// TestLargeScaleScheduling uses a very small number of nodes/pods in CI.
...
// TestConcurrentScheduling uses minimal concurrency for speed.
...
// TestNoGoroutineLeaks uses a minimal number of iterations.
